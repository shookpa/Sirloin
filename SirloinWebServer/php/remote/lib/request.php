 ...
 */
class Request {
	public $restful, $method, $controller, $action, $id, $params,$parametros;
	...
	protected function parseRequest() {
		...
		if (isset($_SERVER["PATH_INFO"])){
			...
			$cap = '/^\/([a-z]+\w)\/([A-Za-z]+\w)\/([A-Za-z0-9-&=\s]+)$/';// /controller/action/parametros
			...
			if (preg_match($cai, $_SERVER["PATH_INFO"], $matches)) {
				...
				$this->id = $matches[3];
			} else if (preg_match($cap, $_SERVER["PATH_INFO"], $matches)) {
				//            	echo "esta entrando";
				$this->controller = $matches[1];
				$this->action = $matches[2];
				//                echo "esta entrando";
				$this->parametros = $matches[3];
